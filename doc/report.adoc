...
[source, python, id="alg-brandes-mod"]
...
Per i test sono stati impiegati una CPU {hardware_local} con frequenza di funzionamento pari a 2.9 Ghz, una cache di 16 Mb e 16 Gb di DRAM con HyperThreading abilitato.

La GPU è una {hardware_nvidia} con quattro _Streaming Multiprocessors_ e clock di base di 2505 Mhz. Ciascuno SM contiene 512 CUDA Core per un totale di 2048 CUDA Cores. La memoria GDDR5 a disposizione è pari a due Gb e la _compute capability_ è 6.1 (architettura Pascal).

I dataset per effettuare i test sono reperibili nella _Sparse Matrix Collection_ dell'Università della Florida cite:[davis_university_2011] e nella _Standford Network Analysis Platform_ (SNAP) cite:[snapnets].

[stripes=even, cols="5,^3,^3,^5,^3,9"]
.Proprietà generali dei dataset considerando la più grande componente connessa
|===
|Grafo       | Nodi  | Archi  | Grado massimo | Diametro | Descrizione

| rnd-er     | 3000  |        |   |   | Grafo casuale Erdös-Rényi
| rnd-sw     | 3000  |        |   |   | Grafo di tipo Small World
| rnd-sf     | 3000  |        |   |   | Grafo di tipo Scale-free
| ca-GrQc    | 4158  | 26850  | ? | ? | Rete di collaborazioni
| ca-HepTh   | 8638  | 49633  | ? | ? | Rete di collaborazioni
| ca-HepPh   | 11204 | 235268 | ? | ? | Rete di collaborazioni
| ca-AstroPh | 17903 | 394003 | ? | ? | Rete di collaborazioni
|===

[stripes=even]
.Proprietà generali dei dataset in seguito alla compressione e considerando la più grande componente connessa
|===
|Grafo       | Nodi  | Archi  | Grado massimo | Diametro
...
Il tempo di esecuzione non include il tempo impiegato per la lettura del grafo
da disco ma tiene conto dell'estrazione della componente connessa, della
compressione e include tutte le comunicazioni tra host e device.
...
[.normalize]
bibliography::bibliography.bib[ieee]
