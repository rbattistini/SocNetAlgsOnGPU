...
module Rouge
  module Lexers
    class Pseudo < RegexLexer
      title "Pseudo"
      desc "Pseudocode language"
      ...
      KEYWORDS = %w(
        ...
        template try tuple type using var when while with without yield procedure
      )
      ...
      state :root do
        ...
        # Keywords
        rule(%r[(#{Pseudo.underscorize(OPWORDS)})\b], Operator::Word)
        rule(/(p_?r_?o_?c_?\s)(?![\(\[\]])/, Keyword, :funcname)
        rule(/(p_?r_?o_?c_?e_?d_?u_?r_?e_?\s)(?![\(\[\]])/, Keyword, :funcname)
        rule(%r[(#{Pseudo.underscorize(KEYWORDS)})\b],  Keyword)
        rule(%r[(#{Pseudo.underscorize(NAMESPACE)})\b], Keyword::Namespace)
        rule(/(v_?a_?r)\b/, Keyword::Declaration)
        rule(%r[(#{Pseudo.underscorize(TYPES)})\b],          Keyword::Type)
        rule(%r[(#{Pseudo.underscorize(PSEUDOKEYWORDS)})\b], Keyword::Pseudo)
        # Identifiers
